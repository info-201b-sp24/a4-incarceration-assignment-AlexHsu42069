...
#code for ggplot
 g2 = ggplot(data = full_state, aes(x=state, y=b_avg, colour=urbanicity, group = urbanicity)) +
   geom_point() +
   geom_line() +
   theme(axis.text.x = element_text(angle = 90)) 
 g2 + labs(title="Average Incarceration rate of the Black population based on state and urbanicity", y="Incarceration Rate (%)", x="State", caption="Data collected from the Vera Institute")
